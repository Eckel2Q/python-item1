...
#IMAGES
BG = pygame.transform.scale(pygame.image.load('images\\prairie_background.png'), (WIDTH,HEIGHT))

# Load 8 animation frames for the bird
BIRD_FRAMES = []
for i in range(8):
    frame = pygame.transform.scale(pygame.image.load(f"images\\bird_frame_{i}.png"), (WIDTH/6, HEIGHT/6))
    BIRD_FRAMES.append(frame)

# Create flipped versions for right-moving birds
BIRD_FRAMES_FLIPPED = []
for frame in BIRD_FRAMES:
    BIRD_FRAMES_FLIPPED.append(pygame.transform.flip(frame, True, False))

CROSSHAIRS = pygame.transform.scale(pygame.image.load("images\\crosshairs.png"), (WIDTH/6, HEIGHT/6))
EXPLOSION = pygame.transform.scale(pygame.image.load("images\\explosion.png"), (WIDTH/6, HEIGHT/6))
DOT = pygame.image.load("images\\shoot_dot.png")
...
# Bird class to handle animation
class Bird:
    def __init__(self, x, y, direction):
        self.rect = pygame.Rect(x, y, WIDTH//6, HEIGHT//6)
    ...
    def get_current_frame(self):
        if self.direction == 'left':
            return BIRD_FRAMES[self.animation_frame]
        else:
            return BIRD_FRAMES_FLIPPED[self.animation_frame]
...
def redraw(player, kill_dot, left_birds, right_birds, score, counter):
    ...
    for bird in left_birds:
        ...
        STAGE.blit(bird.get_current_frame(), (bird.x, bird.y))
